...
def loss(x,l,sum_all=True,adjust_for_variance=False):
    ...
    centered_x = x - means
    if adjust_for_variance is True:
      _, variance = tf.nn.moments(x, [1, 2, 3, 4], keep_dims=True)
      inv_stddev = tf.rsqrt(tf.maximum(variance, 1e-5))
      centered_x = tf.mul(centered_x, inv_stddev*0.1)
...
def sample_from_discretized_mix_logistic(l,nr_mix,bias=None):
    ls = int_shape(l)
    xs = ls[:-1] + [3]
    # unpack parameters
    logit_probs = l[:, :, :, :nr_mix]
    ...
    # if there is bias, sample from biased distribution
    if bias is not None:      
      bias = tf.to_double(bias)
      coeffs = tf.to_double(coeffs)
      log_scales = tf.to_double(log_scales)
      means = tf.to_double(means)
      y0 = bias[:,:,:,0]
      y1 = bias[:,:,:,1] - coeffs[:,:,:,0]*bias[:,:,:,0]
      y2 = bias[:,:,:,2] - coeffs[:,:,:,1]*bias[:,:,:,0] - coeffs[:,:,:,2]*bias[:,:,:,1]
      y = tf.concat(3,[tf.reshape(y0,xs[:-1]+[1]), tf.reshape(y1,xs[:-1]+[1]), tf.reshape(y2,xs[:-1]+[1])])
      center = tf.inv(tf.exp((means - y)*(tf.exp(-log_scales)))+1.0)
      # stddev = 0.3
      # u = tf.truncated_normal(means.get_shape(), center, stddev, dtype=tf.float64)
      # u = center + (u-center)* tf.minimum(center, -center+1.0)/(stddev*2.0)
      u = tf.random_uniform(means.get_shape(), minval=1e-5, maxval=1. - 1e-5, dtype=tf.float64)
      b = y + tf.exp(log_scales)*(tf.log(u) - tf.log(1. - u))
      u = tf.random_uniform(means.get_shape(), minval=1e-5, maxval=1. - 1e-5, dtype=tf.float64)
      x = means + tf.exp(log_scales)*(tf.log(u) - tf.log(1. - u))    
      #choice = tf.round(tf.random_uniform(means.get_shape(), dtype=tf.float64))
      #x = x*choice + b*(-choice+1)
      x = (x + y)*0.5
